...
from build_excel import evol_pct, METIER_SERIES, ZONE_SERIES, REGION_SERIES, ACTIFS_REGION
...
def main():
    ...
    bullet(doc, "Emploi salarié : l'emploi salarié des activités informatiques et services d'information a reculé de 3 % entre fin 2023 et fin 2025, recul porté principalement par les 15-29 ans hors alternants (-3,8 points) ; au 2e trimestre 2026 l'emploi salarié de l'information-communication recule encore de 0,4 % (-3 400 emplois). [Blog du Modérateur/HelloWork citant Insee-Urssaf ; Insee, Informations rapides T2 2026]")
    bullet(doc, "HelloWork (baromètre S1 2026) : le marché de l'emploi tous métiers recule de 9 % au 1er semestre 2026 par rapport au 2e semestre 2025, et de 13 % pour les CDI, après un rebond de +3 % entre fin 2025 et début 2026 ; stocks HelloWork au 18-29 août 2026 : « Développeur informatique » plus de 9 650 offres, « Développeur » plus de 5 700. [HelloWork/HelloWorkplace, juillet 2026]")
    bullet(doc, "Dares (tensions 2024, publication février 2026) : six métiers sur dix en tension forte ou très forte en 2024 contre sept sur dix en 2023 (année record) ; les ingénieurs de l'informatique affichaient en 2023 un indicateur de 1,1 (tension extrême, 67 % de recrutements jugés difficiles) avant de sortir du niveau « très forte » en 2024. [Dares Résultats n° 5, 02/2026 ; Insee Références 2025]")
    bullet(doc, "Perspective structurelle : France Stratégie et la Dares (Métiers 2030) attendent 115 000 postes supplémentaires d'ingénieurs de l'informatique d'ici 2030 (+26 %, première création nette de tous les métiers) ; l'Institut Montaigne chiffre à 190 000 les recrutements d'ingénieurs informatique nécessaires d'ici 2030 et à 845 000 les personnes à former entre 2023 et 2030 ; Eurostat compte 1,4 million de spécialistes TIC en emploi en France en 2025. [France Stratégie/Dares 2022 ; Institut Montaigne 2023 ; Eurostat 05/2026]")
    ...
    table(doc, ["Indicateur", "2022-2023", "2024", "2025", "2026 (au " + DATE_FR + ")", "Source / statut"], [
        ...
        ["Emploi salarié activités informatiques (Insee/Urssaf)", "NC", "en recul (15-29 ans)", "-3 % fin 2023 → fin 2025", "info-com -0,4 % au T2 2026 (-3 400)", "Insee ; Blog du Modérateur (FORTE/MOYENNE)"],
        ["Contrats d'alternance (OPCO Atlas, La bonne alternance)", "NC", "NC", "-14 % (90 719 contrats Atlas)", "entrées en apprentissage -4,8 % au T1 2026 (1re baisse depuis 2018)", "OPCO Atlas 04/2026 ; La bonne alternance T1 2026 (FORTE/MOYENNE)"],
        ["Offres mentionnant l'IA (HelloWork, tous métiers)", "base T1 2023", "NC", "NC", "× 2,4 entre T1 2023 et T1 2026 ; 54 % en Île-de-France", "HelloWork 2026 (MOYENNE)"],
    ...
    ratio_rows = []
    for region_, n_ in reg.most_common():
        if region_ in ACTIFS_REGION:
            n_act, mill, org, url, fiab = ACTIFS_REGION[region_]
            ratio_rows.append((region_, n_, round(100000.0 * n_ / n_act, 1), mill))
    ratio_rows.sort(key=lambda x: -x[2])
    bullet(doc, "Offres pour 100 000 actifs (ESTIMATION sur l'échantillon, population active régionale relevée dans les études Insee ou relais Insee, millésimes 2018-2023, Excel T_ACTIFS_REGIONS) : " + " ; ".join(f"{r_} {v_} (n={n_}, actifs {m_.split(' (')[0]})" for r_, n_, v_, m_ in ratio_rows[:13]) + ". Ce ratio mesure l'intensité de l'échantillon collecté, pas celle du marché total : il est biaisé par l'effort de collecte consacré aux villes NEXA (Pays de la Loire, Auvergne-Rhône-Alpes, Île-de-France) ; les régions sans valeur d'actifs (PACA, Corse, Guyane, Mayotte) restent NC.")
    ...
    for z in NEXA_CITIES:
        rz = zones[z]
        cz, _ = contract_counts(rz)
        kcz = sum(cz.values())
        sz, _ = seniority_counts(rz)
        ksz = sum(sz.values())
        if kcz:
            _alt[z] = pct(cz["ALTERNANCE"] + cz["STAGE"], kcz)
        if ksz:
            _sen[z] = pct(sz["SENIOR"] + sz["LEAD_OU_ARCHITECTE"], ksz)
        _ia_z[z] = ia_share(rz)[1]
    _campus = [z for z in NEXA_CITIES if z != "Marché national à distance"]
    _alt_c = {z: v for z, v in _alt.items() if z in _campus}
    _sen_c = {z: v for z, v in _sen.items() if z in _campus}
    _alt_max = max(_alt_c, key=_alt_c.get) if _alt_c else NC
    _sen_max = max(_sen_c, key=_sen_c.get) if _sen_c else NC
    _alt_min = min(_alt_c, key=_alt_c.get) if _alt_c else NC
    _ia_max = max(_ia_z, key=lambda k: _ia_z[k] if _ia_z[k] != NC else -1)
    _rem = "Marché national à distance"
    p(doc, f"Repères calculés sur l'échantillon (ils priment sur les lectures qualitatives ci-dessus). Parmi les six campus : l'alternance et le stage pèsent le plus à {_alt_max} ({_alt_c[_alt_max]} % des contrats renseignés) et le moins à {_alt_min} ({_alt_c[_alt_min]} %) ; la part de seniors et de leads est la plus forte à {_sen_max} ({_sen_c[_sen_max]} % des séniorités renseignées) ; la mention explicite de l'IA est la plus fréquente à {_ia_max} ({_ia_z[_ia_max]} % des offres de la zone). Le marché national à distance se distingue des six campus : {_alt.get(_rem, NC)} % d'alternance et de stage seulement, {_sen.get(_rem, NC)} % de seniors et de leads. Excel, onglet VILLES_NEXA, table T_COMPARATIF_VILLES.", italic=True)
    ...
    bullet(doc, "Jeunes diplômés : 84 % des Bac+5 de la promotion 2024 ont jugé leur recherche d'emploi difficile (61 % pour la promotion 2022) et 70 % étaient en emploi salarié 12 mois après (-2 points) ; l'enquête CGE 2026 mesure 76 % de diplômés en poste à moins de 6 mois contre 80,2 % un an plus tôt ; les recrutements de cadres débutants en informatique ont reculé de 19 % en 2024. [Apec 11/2025 et 2026 ; CGE via Diplomeo 06/2026]")
    bullet(doc, "Alternance : OPCO Atlas enregistre -14 % de contrats d'alternance en 2025 (90 719 nouveaux contrats dans ses branches, dont le numérique) ; La bonne alternance observe au 1er trimestre 2026 la première baisse des entrées en apprentissage depuis 2018 (-4,8 %) ; l'Insee note un recul des contrats d'alternance au 4e trimestre 2025 (-0,8 %). Le canal alternance reste le premier accès des débutants, mais il se resserre. [OPCO Atlas 04/2026 ; La bonne alternance T1 2026 ; Insee 03/2026]")
    bullet(doc, "Stocks « junior » : HelloWork n'affiche que 11 offres « développeur web junior » (3 septembre 2026) et 14 « développeur informatique junior » (26 août 2026) pour plus de 5 700 offres « développeur » ; Indeed compte plus de 100 « Développeur Web Junior » actives en France pour plus de 3 000 « Développeur Web » (septembre 2026), mais « Développeur Junior » (tous domaines) restait à plus de 1 000 fin 2025. [pages publiques HelloWork et Indeed, Excel T_STOCKS_NATIONAUX]")
    ...
    bullet(doc, "Indeed Hiring Lab (juillet 2026) : la France comptait 138 intitulés de postes « touchés par l'IA » au 1er trimestre 2026, soit 3,3 % des intitulés, dont 54 % hors métiers tech ; HelloWork observe des offres mentionnant l'IA multipliées par 2,4 entre le 1er trimestre 2023 et le 1er trimestre 2026, l'Île-de-France captant 54 % de ces offres ; LinkedIn place l'ingénieur IA en tête de son classement 2026 des métiers en croissance en France. [Indeed Hiring Lab 08/07/2026 ; HelloWork 2026 ; LinkedIn Jobs on the Rise 2026]")
    bullet(doc, "Études françaises sur l'IA et l'emploi : l'OPIIEC (OPCO Atlas) estime que l'IA générative pourrait créer 45 000 emplois en trois ans dont 31 600 dans le numérique ; Coface et l'Observatoire des emplois menacés jugent 3,8 % de l'emploi français fragilisé par l'IA générative et 16,3 % des salariés exposés à 2-5 ans ; la Direction générale du Trésor (Trésor-Éco n° 391, juin 2026) conclut à un effet net agrégé « incertain » entre substitution et gains de productivité ; Bpifrance mesure 55 % de PME utilisatrices d'IA générative fin 2025 (31 % un an plus tôt). [OPIIEC 2025 ; Coface/OEM 01/04/2026 ; DG Trésor 30/06/2026 ; Bpifrance 01/2026]")
    ...
    bullet(doc, "International, effet sur les juniors (éclairage, pas de conclusion France) : Stanford (données ADP) mesure un recul de près de 20 % de l'emploi des développeurs de 22-25 ans aux États-Unis entre fin 2022 et juillet 2025 ; une étude Harvard (SSRN, août 2025) observe que les entreprises adoptant l'IA générative réduisent l'emploi junior de 7 à 12 % tandis que l'emploi senior continue de croître, par ralentissement des embauches et non par licenciements ; Indeed Hiring Lab (juillet 2026) constate que 71 % de la hausse des offres de développement logiciel entre mai 2025 et mai 2026 provient de postes seniors, les offres restant 27,5 % sous leur niveau d'avant la pandémie. [Stanford Digital Economy Lab 11/2025 ; Harvard/SSRN 08/2025 ; Indeed Hiring Lab 07-08/2026]")
    ...
    H = [
        ...
        ["H10. Les différences territoriales justifient une stratégie distincte selon les campus NEXA.", "PARTIELLEMENT_CONFIRMEE", f"Paris = 29 % des offres Apec et {pct(reg.get('Île-de-France', 0), len(U))} % de l'échantillon, ville qui concentre l'essentiel des offres IA ({pct(sum(1 for r in U if r['FAMILLE_METIER'] == IA and r['ZONE_NEXA'] == 'Paris et Île-de-France'), max(1, fam[IA]))} % des offres IA de l'échantillon) ; Lyon 300-400 offres « développeur » actives, Marseille ≈ 100 ; Lille alternance et e-commerce, Nantes orientée ESN et profils expérimentés ; Apec prévisions régionales +2 à +6 %.", "collecte ; Apec régional ; BMO 2026", "Les compétences demandées sont homogènes d'une ville à l'autre : la stratégie diffère par le volume, l'alternance et les partenaires, pas par le contenu.", "MOYENNE", "Socle commun national, déclinaison locale des partenariats et du volume d'admission."],
    ]
